...
/// In debug builds, box parsers to shrink combinator types during type-checking.
/// Release keeps concrete types (no boxing) for the benchmark path.
macro_rules! erase_types_in_debug {
    ($parser:expr) => {{
        let p = $parser;
...
pub fn get_json_grammar<'src>() -> impl Parser<'src, &'src str, Output = JsonValue<'src>> + Clone {
    recursive(|element| {
        ...
        let raw_string = erase_types_in_debug!(
            Rc::new(
                capture!({
                    commit_on(
                        '"',(
        ...
        let array = erase_types_in_debug!(capture!({
            commit_on((ws.clone(), '['),
            (
                ws.clone(),
                optional((
        ...
        let key_value_pair = erase_types_in_debug!(Rc::new(
            capture!({
                (
        ...
        let object = erase_types_in_debug!(capture!({
                commit_on((ws.clone(), '{'),
                (
                ws.clone(),
                optional((
        ...
        let invalid_element = erase_types_in_debug!(
            capture!(
